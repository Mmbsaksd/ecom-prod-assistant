...
from prod_assistant.etl.data_scapper import FlipkartScraper
...
def add_product_input():
    st.session_state.product_input.append("")
...
updated_inputs = []
for i, val in enumerate(st.session_state.product_input):
...
review_count = st.number_input("How many reviews per product?", min_value=1, max_value=10, value=1)

if st.button("🚀 Start Scraping"):
    product_input = [p.strip() for p in st.session_state.product_inputs if p.strip()]
    if product_description.strip():
        product_input.append(product_description.strip())
    ...
    if not product_input:
        ...
        for query in product_input:
            ...
            result = flipkart_scraper.scrape_flipkart_products(query,max_product=max_products, review_count=review_count)
        ...
        for row in final_data:
            ...
            st.session_state["scrapped_data"] = final_data
